...
void GS(point v2g InData[1], inout TriangleStream<g2p> triStream)
{
    ...
    float3 pos1 = InData[0].position.xyz + (right - up) * size * 0.5;
    float3 pos2 = InData[0].position.xyz + (right + up) * size * 0.5;
    float3 pos3 = InData[0].position.xyz - (right + up) * size * 0.5;
    float3 pos4 = InData[0].position.xyz - (right - up) * size * 0.5;
    ...
    for (int j = 0; j < 4; j++) {
        newPoints[j].position = mul(newPoints[j].position, c_cameraSpaceViewProj);
...
{
    float4 color = ParticleTexture.Sample(DefaultSampler, pIn.uv);
