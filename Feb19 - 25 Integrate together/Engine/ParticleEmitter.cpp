...
void ParticleEmitter::Render()
{
	...
	UINT offset = 0;
	pGraphics->UploadBuffer(mPartBuffer, &mPartRender, /*sizeof(mPartRender)*/sizeof(mPartRender[0]) * particleNumber);
	pGraphics->GetDeviceContext()->IASetVertexBuffers(0, 1, &mPartBuffer, &stride, &offset);
	//draw call
	pGraphics->GetDeviceContext()->Draw(/*sizeof(mPartRender) / sizeof(mPartRender[0])*/particleNumber, 0);
...
{
	for (int i = 0; i < particleNumber; i++) {
		mPartRender[i].pos.z += IntRandomGenerator(10, 50);
