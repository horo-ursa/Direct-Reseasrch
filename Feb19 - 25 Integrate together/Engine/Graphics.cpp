...
void Graphics::InitD3D(HWND hWnd, float width, float height)
{
    ...
    //setup and create depthstencil state
    dsState = CreateDepthStencilState(D3D11_COMPARISON_LESS);
    mDepthAlphaState = CreateDepthStencilState(D3D11_COMPARISON_LESS_EQUAL);
    mDevCon->OMSetDepthStencilState(dsState, 0);
...
{
    mDevCon->OMSetRenderTargets(1, &mBackBuffer, DepthView);
    mDevCon->OMSetDepthStencilState(dsState, 0);
    mDevCon->ClearDepthStencilView(DepthView, D3D11_CLEAR_DEPTH, 1.0f, 0);
